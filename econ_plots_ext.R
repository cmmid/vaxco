...
base_id <- epi_scen.dt[#setId==0 &
                           ...
                           vax_eff==0.7 &
                           nat_imm_dur_days==1825 &
                           vax_imm_dur_days==1825 &
                           start_timing==18718 &
...
to.add <- base.list
to.add$scen_name = "2.5 year vaccine & natural immunity waning"
to.add$nat_imm_dur_days = 912
to.add$vax_imm_dur_days = 912
scen.list <- rbindlist(list(scen.list, to.add))
...
to.add <- base.list
to.add$scen_name = "1 dose regimen"
to.add$vax_delay = 0 
scen.list <- rbindlist(list(scen.list, to.add))
...
scen.tab <- scen.dt[,.("Scenario No."=scen_id,"Description"=scen_name,
                       "Difference in Cost ($ millions)" = sprintf(
                           "%.1f (%.1f, %.1f)",
                           ccosts_md/10^6,ccosts_hi95/10^6,ccosts_lo95/10^6
                           ),
